...
import { ArrowLeft, CheckCircle, ArrowRight, Camera, Globe, Copy, Check } from 'lucide-react';
...
const DetailPage: React.FC<DetailPageProps> = ({ data, onBack, onContact }) => {
  ...
  const getSubdomain = (id: string) => {
    const map: Record<string, string> = {
        'digital-employees': 'staff',
        'marketing': 'growth',
        'webdesign': 'web',
        'ki-video': 'studio'
    };
  ...
  const subdomainUrl = `https://${getSubdomain(data.id)}.swiss-ai.expert`;
            ...
            <div className="relative z-20">
                <ScrollReveal animation="fade-in-right">
                    ...
                    <div 
                        ...
                        className="inline-flex items-center gap-2 mb-6 px-4 py-2 rounded-full bg-slate-800/60 border border-slate-700 hover:border-blue-500/50 cursor-pointer transition-all group/link backdrop-blur-md shadow-lg"
                        title="Link kopieren"
                    >
                        <Globe size={14} className="text-blue-400" />
                        <span className="text-xs font-mono text-blue-200 tracking-wide">{subdomainUrl}</span>
                        <div className="pl-3 border-l border-white/10 ml-1">
                             {copied ? <Check size={14} className="text-green-400" /> : <Copy size={14} className="text-gray-500 group-hover/link:text-white transition-colors" />}
                        </div>
                    </div>
